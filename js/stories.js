...
function generateStoryMarkup(story) {
  ...
  // if user is logged in, show star. if favorites, apply fas class
  return $(`
      <li id="${story.storyId}">
        <i class="fa-star"></i>
...
function putFavoritesOnPage() {
  ...
  if (favorites.length >= 1) {
    ...
    const $noFavoriteMsg = $('<h5 id="no-favorite-stories-msg">No favorites added!</h5>');
...
function putUserStoriesOnPage() {
  ...
  if (userStories.length >= 1) {
    ...
    const $noStoriesMsg = $('<h5 id="no-user-stories-msg">No stories added by user yet!</h5>');
